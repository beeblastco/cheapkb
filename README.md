...
The workspace follows a two-column layout: one searchable, sortable document table with exact pipeline status values on the left and a shadcn message-scroller chat with related sources on the right. Staged files appear directly in the document table, the table shows 50 rows per page, and `Sync all` starts the staged batch. The user menu reads the signed Google profile and provides account, policy, contact, and logout actions.
