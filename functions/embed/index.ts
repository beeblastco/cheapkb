...
async function processBatch(
  ...
  for (let i = 0; i < embeddings.length; i++) {
    ...
    if (meta.skipped || !meta.chunkId) continue;
    vectorBatch.push({
      key: meta.chunkId,
      data: embeddings[i],
      metadata: {
  ...
  for (const vector of vectorBatch) {
    ...
    const changed = await markChunkEmbedded(meta.documentId, meta.chunkId);
    if (changed) {
      docCounts[meta.documentId] = (docCounts[meta.documentId] ?? 0) + 1;
      const tokens = meta.tokenCount ?? 0;
      usageByUser[meta.userId] = (usageByUser[meta.userId] ?? 0) + tokens;
    }
  }
  ...
  for (const [userId, count] of Object.entries(usageByUser)) {
    await recordUsage(userId, AccountsTableName, "embed", count);
  }
